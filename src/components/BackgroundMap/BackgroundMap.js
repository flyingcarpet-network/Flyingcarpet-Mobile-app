...
import { type MapCoordinateCircleType, type MapCoordinateType } from '../../types';
...
type Props = {
  ...
  selectedLocationCoordinates: {[string]: MapCoordinateType},
  displayCloseButton: boolean, // This is an optional prop that can be passed in
  disablePolygonCreation: boolean, /* This is an optional prop that disables the ability to create
                                      a polygon/markers by pressing on the map */
  onMapPress: () => {}, /* This is a custom callback function that is run whenever the map is
                           pressed (not related to polygon creation functionality). */
                        /* NOTE: When this callback is defined, the user cannot interact with the
                           map at all (since any press on the map triggers the callback). */
  showFlyingCarpetBestLocations: boolean, /* This is an optional prop that makes it so that the map
                                             also display the ideal (most profitable) locations to
                                             place flying carpets */
  drawLine: boolean /* If passed, instead of a polygon a line will be drawn connecting the markers
                       in the order they were added (a polygon is drawn by default if this argument
                       is not passed) */
};
...
class BackgroundMap extends React.Component<Props> {
  ...
  componentDidMount() {
    setTimeout(() => { // 1/1000 second timeout prevents timing issue on Android
      ...
      if (this.map != null && this.map.fitToCoordinates != null &&
          Object.keys(selectedLocationCoordinates).length > 0) {
  ...
  handleMapPress = (e: ResponderSyntheticEvent<NativeTouchEvent>): void => {
    ...
    // If "coordinate" is not defined, then it means the marker was pressed ("coordinate" is not
    // passed when the marker is directly pressed), thus we don't add this marker.
    // NOTE: this happens because the MapView's onPress callback is called regardless of whether
    // the press event also trigger the Marker's onPress callback also.
    if (!('coordinate' in e.nativeEvent)) {
      return;
    }

    // If polygon creation isn't disabled, add the pressed location to the polygon coordinates
    // array
    if (!disablePolygonCreation) {
      // Add coordinate (lat/long pair) to redux for the user's pressed location and include
      // an "identifier"
  ...
  handleMarkerPress = (e: ResponderSyntheticEvent<NativeTouchEvent>): void => {
    ...
    // NOTE: We don't need to check if polygon creation is disabled, since markers are only show
    // if it is disabled

    // Remove the coordinate (lat/long pair) from redux for the marker at the location that the
    // user pressed
  ...
  render(): React.Node {
    ...
    return (
      <View style={styles.mapWrap}>
        <MapView
          style={styles.map}
          // NOTE: this initialRegion should be updated to get the user's current location and use
          // it, see:
          ...
          ref={(ref: ?{fitToCoordinates: Array<MapCoordinateType> => {}}) => { this.map = ref; }}
          ...
          // We use the Google provider since it also allows styling of the map (without needing to
          // install anything beyond Expo)
          provider="google"
          ...
          // To enable iOS custom styles (may need to eject, not sure), see:
          ...
          // Eslint must be disabled for the next line since the map JSON is very long
        ...
        >
          ...
          {(!disablePolygonCreation) &&
           (Object.keys(selectedLocationCoordinates).length >= 2) && /* Only show polygon region if
                                                                        some coordinates have been
                                                                        added */
           <PolygonOrLine
          ...
          {/* If polygon creation isn't disabled, display the markers on the map that outline the
              polygon */}
          {(!disablePolygonCreation) &&
           getValues(selectedLocationCoordinates).map((coordinate: MapCoordinateType) => {
            ...
            return (
              <MapView.Marker
                key={uniqueID} /* We need to ensure each marker has a unique "key" value so that
                                  React can diff efficiently (since markers are added and
                                  removed) */
          ...
          {/* If the functionality to display the best (most profitable) flying carpet locations
              is enabled, then we display all of the best location circles and markers */}
          {showFlyingCarpetBestLocations &&
           flyingCarpetBestLocations.map((coordinate: MapCoordinateCircleType) => {
            const key = coordinate.label.replace(/\W/g, '');
            return (
              // Circles
              <MapView.Circle
                key={`bestFCLocationCircle_${key}`}
                radius={1500}
                center={coordinate}
          ...
          {showFlyingCarpetBestLocations &&
           flyingCarpetBestLocations.map((region: MapCoordinateCircleType) => {
            const key = region.label.replace(/\W/g, '');
            return (
              // Markers
              <MapView.Marker
                key={`bestFCLocationMarker_${key}`}
                coordinate={region}
                anchor={{ x: 0.5, y: 0.5 }}
              >
        ...
        {displayCloseButton &&
          ...
          >
            <FontAwesome
              name={mapOpen ? 'angle-right' : 'angle-left'}
              size={25}
              style={styles.leftArrowText}
            />
          </TouchableHighlight>
        }
        {(displayCloseButton && mapOpen) && /* If we're on the "increase revenue" dialog OR if
                                               the map is not minimized (on the business
                                               dialogs) */
          <GooglePlacesAutocomplete
            placeholder="Search"
            minLength={2} // minimum length of text to search
            autoFocus={false}
            // Below can be left out for default return key
            ...
            onPress={(
              ...
              // Must ensure that the map instance instance variable and details argument exist (to
              // prevent flow errors)
              if (this.map != null && this.map.fitToCoordinates != null && details) {
            ...
            nearbyPlacesAPI="GooglePlacesSearch" /* Which API to use: GoogleReverseGeocoding or
                                                    GooglePlacesSearch */
...
export default connect(
  ...
  dispatch => ({
    ...
    removeLocationCoordinate: bindActionCreators(
      businessActions.removeLocationCoordinate,
      dispatch,
    ),
  }),
)(BackgroundMap);
