/*
 * This is the flyingcarpet owner channels scene where the user can view the open state channels
 * that their flyingcarpet is conencted to.
 * @flow
 */
...
class FlyingCarpetOwnerChannels extends React.Component<Props> {
        ...
        */}
        <List containerStyle={styles.list}>
          ...
          {channels.map((channel: ChannelType): React.Node => (
            <TouchableOpacity
              key={`channel_${channel.partnerAddress}`}
              onPress={() => this.handleChannelTypePress(channel)}
            >
              <ListItem
